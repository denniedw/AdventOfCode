...
var fs = require("fs");
var PartOne = function (rangeData, ids) {
    var freshIngredients = [];
    ids.forEach(function (id) {
        rangeData.forEach(function (range) {
            var leftLine = +range.split('-')[0];
            var rightLine = +range.split('-')[1];
...
var PartTwo = function (rangeData) {
    // remove ranges that fall between range of other range
    var needAnotherIteration = true;
    var freshIngredientsIds = 0;
    var ranges = [];
    // create ranges
    rangeData.forEach(function (range) {
        var leftLine = +range.split('-')[0];
    ...
    var _loop_1 = function (i) {
        var range = ranges.find(function (r) { return r.left === ranges[i].left && r.right === ranges[i].right; });
        if (range) {
    ...
    // remove -1 -1 ranges
    uniqueRanges = uniqueRanges.filter(function (r) { return !(r.left < 0 && r.right < 0); });
    console.log(uniqueRanges);
    uniqueRanges.forEach(function (range) {
        freshIngredientsIds += (range.right - range.left) + 1;
    });
    console.log(uniqueRanges.sort(function (a, b) { return a.left - b.left; }));
...
var GetData = function () {
    ...
    fs.readFileSync('input.txt', 'utf-8').split('\n').map(function (line) {
        if (line.split('').find(function (c) { return c === '-'; })) {
    ...
    return { rangeData: rangeData, ids: ids };
};
var _a = GetData(), rangeData = _a.rangeData, ids = _a.ids;
